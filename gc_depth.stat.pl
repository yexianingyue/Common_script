...
if ($color==0) {
	plot(data.uq[,1:2],pch=46,col=hcl(h=1,c=data.col),xlim=c(0,100),ylim=c(0,$max_len),axes=F)
} else {
	plot(data.uq[1,1:2],pch=46,col="red",xlim=c(0,100),ylim=c(0,$max_len),axes=F)
    $color_code
}
axis(side=1,seq(0,100,10))
axis(side=2,seq(0,$max_len,$max_len/10))
