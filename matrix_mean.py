...
# ModiffTime:       2020年11月06日 星期五 16:01:20 CST
###########################################################
'''
Version : V3.0
可以求分组的平均值
...
def main(file_, sstr, output_file):
    ...
    for line in f:
        ...
        # 格式化每一行的数据
        num = [float(x)/ngroup for x in line_split[1:]] # 如果需要整数，此处改为int即可
        for group in groups:
            if result.get(group):
                result[group] = list(map(lambda x, y: x + y, result[group], num)) # ！！！！！！！！！！！这边不需要赋值，因为赋值会得到空值原理不清楚
            else:
                result[group] =  num
    f.close()
