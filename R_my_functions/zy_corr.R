...
zy_corr = function(x, y=NA, method="spearman", type="matrix"){
  ...
  if(!is.na(y)){
    ...
    if(type=="matrix"){
      ...
      for(i in 1:num_x){
        ...
        for(j in 1:num_y){
          name_j = name_y[j]
          ct = cor.test(as.numeric(x[name_i,]), as.numeric(y[name_j,]), emthod=method)
      ...
      for(i in 1:num_x){
        ...
        for(j in 1:num_y){
          name_j = name_y[j]
          ct = cor.test(as.numeric(x[name_i,]), as.numeric(y[name_j,]), method=method)
    ...
    if(type=="matrix"){
      ...
      for(i in 1:num_x){
        ...
        for(j in i:num_x){
          name_j = name_x[j]
          ct = cor.test(as.numeric(x[name_i,]), as.numeric(x[name_j,]), method=method)
          cor_result[name_i,name_j] = cor_result[name_j,name_i] = ct$estimate
          p_result[name_i,name_j] = p_result[name_j, name_i]= ct$p.value
        }
      }
