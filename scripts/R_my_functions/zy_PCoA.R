...
get_margin <- function(main_plot){
  library(patchwork)
  
  expands=c(0.1, 0.1)
  main_plot <- main_plot+theme(aspect.ratio = 1)+
    scale_x_continuous(expand = expands) +
    scale_y_continuous(expand = expands)
  
  xx = ggplot_build(main_plot)
  xlims = xx$layout$panel_scales_x[[1]]$range$range
  ylims = xx$layout$panel_scales_y[[1]]$range$range
  
  myfill = main_plot@scales$scales[[1]]$palette(1)
  
  px <- main_plot$data %>%
    ggplot(aes(x = pcoa.1, y = group, fill=group)) +
    geom_boxplot() +
    scale_x_continuous(limits=xlims, expand =expands)+
    scale_fill_manual(values=myfill)+
    theme_minimal() +   # 或你喜欢的主题
    labs(x = NULL, y = NULL) +          # 去掉标签，避免重复
    theme(axis.text.y = element_blank(),   # 隐藏 y 标签（因为主图已有）
          axis.ticks.y = element_blank())  # 调整右边距，让它紧贴主图
  
  
  # 垂直 boxplot（下边）：y = pcoa.2, x = group
  py <- main_plot$data %>%
    ggplot(aes(y = pcoa.2, x = group, fill=group)) +
    geom_boxplot() +
    scale_fill_manual(values=myfill)+
    scale_y_continuous(limits=ylims, expand = expands)+
    theme_minimal() +
    labs(x = NULL, y = NULL) +
    theme(axis.text.x = element_blank(),   # 隐藏 x 标签
          axis.ticks.x = element_blank())  # 调整上边距
