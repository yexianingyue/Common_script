...
zy_compositions <- function(dt=NA, top_N = 10, 
  ...
  ## default  
  if(is.null(themes)){
    themes  = theme_bw() + theme(panel.grid = element_blank(), axis.text.x = element_text(angle=45, hjust=1, vjust=0.5))
  }
...
zy_group_compositions <- function(dt=NA, sample_map=NA, ID="Sample", group="Group", top_N = 10, 
    ...
    # order_func -> 按照什么对样本排序
  if(is.null(themes)){
    themes  = theme_bw() + theme(panel.grid = element_blank(),
    ...
    if(is.null(group_order)){
      group_order = unique(dm[,group])
    }
    dm[,group] = factor(dm[,group], level=group_order)
