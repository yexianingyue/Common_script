...
def get_args():
    ...
    parser.add_argument('-t', default=0, type=float, help=f"with title. [0]")
...
def main(ani_file, outf, cov_cut, ani_cut, linkage_method='average', title=0):
    ...
    if title == 0:
        df = pd.read_csv(ani_file, sep="\t", header=None, usecols=[0,1,2,3], dtype = {0:str, 1:str, 2:float, 3:float})
    else:
        df = pd.read_csv(ani_file, sep="\t", header=0, usecols=[0,1,2,3], dtype = {0:str, 1:str, 2:float, 3:float})

    df.columns=['ref','query','ani', 'ref_align']

    df['ani'] = df['ani']/100
    df['cov'] = df['ref_align']/100 # skani也是百分比覆盖度
...
if __name__ == "__main__":
    args = get_args()
    main(args.i, args.o, args.nc, args.ani, args.linkage_method, args.t)
