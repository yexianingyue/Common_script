...
# 初始化默认值
threads=8
conda_env="/share/data1/software/miniconda3/envs/dvf"
bin_path="/share/data1/lvqb/software/DeepVirFinder/dvf.py"
force=0 # 是否强制重跑
clean=0 # 是删除临时文件
...
echo "Run deepvirfinder"
python ${bin_path} -i ${input} -o ${output} -c ${threads}

echo "extract virus"
awk -F "\t" '$4<0.01 && $3>=0.9' $(find ${output} -name "*bp_dvfpred.txt") | cut -d " " -f 1 > ${output}/virus.name
