...
def get_args():
    ...
    parser.add_argument("-e", metavar="warning", required=False, type=bool, default=False, help="warning.(False)")
...
def main(file_, sstr, output_file, title, error):
    result = {}
    ...
    for line in f:
        ...
        try:
            ...
            for i,v in enumerate(line_split[1:]):
                try:
                    float(v)
                except Exception as e:
                    xs = re.split(sstr, tittle)
                    print(xs[i+1])
                    print(f"i:{i}\tv:{v}")
                    raise
        for group in groups:
...
if __name__ == "__main__":
    ...
    main(args.i, sstr, args.o, args.t, args.e)
    if args.N == args.G < 0:
