...
if [ $# -ne 2 ];then
    ...
    echo -e  "  -c columns"
    echo -e  "  -r row"
    echo -e  "  -R row(exclude 0)"
    echo -e  "  -C columns(exclude 0)"
    exit 0
fi

while getopts "crRC" arg;
do
    case $arg in
        r)
        ...
        R)
            ...
            ;;
        C)
            perl -e 'open I, "$ARGV[0]";%line;$t=readline, I;chomp($t);@x=split(/\t/,$t);%h;while(<I>){chomp;@l=split/\t/;foreach $k(1..@l-1){$h{$x[$k]}+=$l[$k]; $line{$x[$k]}++ if $l[$k] != 0} }; foreach $k(keys %h){$c=$line{$k}; if( $c > 0){$ave=$h{$k}/$c }else{$ave=0};print "$k\t$h{$k}\t$ave\t$c\n"}' $2
            ;;
        \?)
            echo "other"
            ;;
