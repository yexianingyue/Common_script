...
def calc_wilcox_pvalue(p_dt, p_map, sample="Sample", group="Group"):
    ...
    # 删除掉分组为NA的行
    mask = p_map[group].isnull()
    p_map = p_map[~mask]
    
    # 删除重复的数据
    p_map = p_map.loc[:,[sample,group]].drop_duplicates()
    p_map.index = p_map[sample]

    # 对齐
    intersect_id = p_dt.columns.intersection(p_map[sample])
    p_map = p_map.reindex(index=intersect_id)
    p_dt = p_dt.reindex(columns=intersect_id)
    
    print("filtered abundance.")
    print(p_dt.shape)
    ...
    for n in range(0, nspecies) :
        sys.stdout.write(f"\rnspecies:{n+1}/{nspecies}")
...
def get_args():
    ...
    parser.add_argument("-g", metavar="group", required=True, type=str, help="sample group file. [header needed]")
    parser.add_argument("-n", metavar="id", default="Sample", type=str, help="ID of sample")
    parser.add_argument("-G", metavar="Group", default="Group", type=str, help="Group of sample")
...
def main(in_d, in_map, out_f, sample="Sample", group="Group", sstr="\t"):
    print("read table")
    dt = pd.read_csv(in_d, sep=sstr, header=0, index_col=0,engine='python', encoding="unicode_escape")
    sample_map = pd.read_csv(in_map, sep=sstr, header=0, engine='python', encoding="unicode_escape")
...
if __name__ == "__main__":
    ...
    main(in_d = args.d, in_map= args.g, sample=args.n, group=args.G, out_f=args.o, sstr=sstr)
