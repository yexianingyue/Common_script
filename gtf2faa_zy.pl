...
my $gtf = $ARGV[1];
my $prot = $ARGV[2];
my $prot_seq = $ARGV[3]; # zhangy2

$usage = "Author: Katharina J. Hoff\nModiff: Zhangy2  2020-2-6\n\nUsage: gtf2aa.pl genome.fa genes.gtf prot.fa prot.fna\n"; # zhangy2

if(scalar(@ARGV)!=4){ # zhangy2
    print $usage;
    exit(1);
}

gtf2fasta($genome, $gtf, $prot, $prot_seq); # zhangy2
...
sub gtf2fasta {
    ...
    my $gtf_file = shift;
    my $fasta_file = shift;
    my $prot_seq_file = shift; # zhangy2
    my %gtf;
    my %genome;
    ...
    close(GENOME) or die ("ERROR: in file " . __FILE__ ." at line ". __LINE__
              . "\nCould not close file $genome_file!\n");
#==================================================================================================
# write fna of protein to the file  by zhangy2
#==================================================================================================
    open (FASTA, ">", $prot_seq_file) or die ("ERROR: in file " . __FILE__ ." at line " . __LINE__
                       . "\nCould not close file $prot_seq_file!\n");
