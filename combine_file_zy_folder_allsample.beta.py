...
def get_args():
    ...
    parser.add_argument('-F', metavar='format',type=str, default='float', choices=['fraction', "float"], help='\033[31mdata format. [int, fraction, float:default]\033[0m')
...
def main(sstr, input_Dir, output_file, suffix, f_title, position, format_function, out_format):
    """循环列表"""
    global STRAIN_LIST
    global STRAIN_INDEX_DICT
    global INIT_CAZY
        ...
        if out_format == "float":
            for k, v in result_dict.items():
                ...
                if sum(v_temp) == 0:
                    ...
                    continue
                v = sstr.join([str(x) for x in v_temp])
                f.write(f"{k}{sstr}{v}\n")
        else:
            for k, v in result_dict.items():
                v_temp = [format_function(x) for x in v]
...
if __name__ == "__main__":
    ...
    main(sstr, args.D, args.o, args.suffix, f_title, positions, format_function, out_format)
